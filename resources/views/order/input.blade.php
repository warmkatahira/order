...
<x-app-layout>
    ...
    <div class="py-3 px-4 grid grid-cols-12">
        @if(session('order_info'))
            <form method="post" action="{{ route('order.confirm') }}" id="order_form" class="m-0 col-span-12 grid grid-cols-12">
                @csrf
                ...
                <div class="col-span-12 grid grid-cols-12 gap-4 grid-rows-6">
                    ...
                    <div class="row-span-3 col-span-12 xl:col-span-3 grid grid-cols-12 gap-4 border border-black rounded-lg p-3 bg-emerald-100">
                        ...
                        <div class="col-span-12 grid grid-cols-12">
                            ...
                            <input type="date" id="delivery_date" name="delivery_date" class="rounded-lg col-span-7 text-sm" autocomplete="off" value="{{ old('delivery_date') }}">
                        </div>
                        <div class="col-span-12 grid grid-cols-12">
                            ...
                            <select id="delivery_time" name="delivery_time" class="rounded-lg col-span-7 text-sm">
                                @foreach(App\Consts\DeliveryTimeConsts::TIME_LIST as $key => $value)
                                    <option value="{{ $key }}" {{ $key == old('delivery_time') ? 'selected' : '' }}>{{ $value }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-span-12 grid grid-cols-12">
                            ...
                            <select id="shipping_method" name="shipping_method" class="rounded-lg col-span-7 text-sm">
                                @foreach(App\Consts\ShippingMethodConsts::SHIPPING_METHOD_LIST as $key => $value)
                                    <option value="{{ $key }}" {{ $key == old('shipping_method') ? 'selected' : '' }}>{{ $value }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
        ...
        @else
            <p class="col-span-12">発注商品が選択されていません</p>
        @endif
    </div>
    <!-- 配送店舗選択モーダル -->
