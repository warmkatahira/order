...
<x-app-layout>
    <x-slot name="header" class="">
        <div class="grid grid-cols-12 gap-4">
            ...
            @if($order->order_status == '発注待ち' && Auth::user()->role_id <= '11')
                <a href="{{ route('order_modify.index', ['order_id' => $order->order_id]) }}" id="order_modify" class="col-start-11 col-span-1 rounded-lg text-center py-2 bg-orange-200">修正画面へ</a>
                <a href="{{ route('order.cancel', ['order_id' => $order->order_id]) }}" id="order_cancel" class="col-start-12 col-span-1 rounded-lg text-center py-2 bg-pink-200">キャンセル</a>
            @endif
        </div>
    </x-slot>
    <div class="py-3 px-4 grid grid-cols-12">
        <form method="POST" action="{{ route('order_status_modify') }}" class="m-0 col-span-12 grid grid-cols-12">
            @csrf
            <p class="text-xl border-b-4 border-blue-500 col-span-2 mb-3">ステータス変更</p>
            <div class="col-span-12 grid grid-cols-12 mb-3">
                <!-- ステータス条件 -->
                <label for="order_status" class="col-span-1 text-sm text-center py-2 bg-black text-white">ステータス</label>
                <select id="order_status_select" name="order_status_select" class="col-span-1 text-sm">
                    @foreach(App\Consts\OrderStatusConsts::ORDER_STATUS_LIST_MODIFY_TARGET as $key => $value)
                        <option value="{{ $key }}">{{ $value }}</option>
                    @endforeach
                </select>
                <button type="submit" id="order_status_modify" class="col-span-1 rounded-lg bg-blue-200 ml-5">変更</button>
                <input type="hidden" name="order_id" value="{{ $order->order_id }}">
            </div>
        </form>
        <div class="col-span-12 grid grid-cols-12 grid-rows-6 gap-4">
            <!-- 発注概要 -->
            <div class="row-span-6 col-span-3 grid grid-cols-12 p-5 border border-black rounded-lg bg-emerald-100">
                <p class="col-span-12 text-sm">ステータス</p>
                <p class="col-span-12 text-3xl text-center">{{ $order->order_status }}</p>
            </div>
            <!-- 発注概要 -->
            <div class="row-span-6 col-span-3 grid grid-cols-12 p-5 border border-black rounded-lg bg-emerald-100">
        ...
        <table class="col-start-1 col-span-12 mt-5">
            <thead>
                <tr class="text-sm text-left text-white bg-gray-600 border-gray-600">
                    ...
                    <th class="font-thin p-2 px-2 w-1/12 text-right">発注数</th>
                </tr>
            </thead>
