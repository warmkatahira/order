...
<x-app-layout>
    <x-slot name="header" class="">
        <div class="grid grid-cols-12 gap-4">
            ...
            <a href="{{ route('shipping_data_download') }}" class="col-start-11 col-span-1 text-sm text-center bg-orange-200 rounded-lg py-3">出荷データDL</a>
            <a href="{{ route('shipping_actual.index') }}" class="col-start-12 col-span-1 text-sm text-center bg-orange-200 rounded-lg py-3">出荷実績</a>
        </div>
    </x-slot>
    <div class="py-3 px-4 grid grid-cols-12">
        ...
        <form method="GET" action="{{ route('order_list.search') }}" class="m-0 col-span-12 grid grid-cols-12">
            ...
            <div class="col-span-12 grid grid-cols-12 mb-3">
                <!-- 発注日条件 -->
                <label for="order_date" class="col-span-1 text-sm text-center py-2 bg-black text-white">発注日</label>
                <input type="date" id="order_date_from" name="order_date_from" class="text-xs col-span-1" value="{{ session('order_date_from') }}">
                <p class="col-span-1 text-sm text-center py-2">～</p>
                <input type="date" id="order_date_to" name="order_date_to" class="text-xs col-span-1" value="{{ session('order_date_to') }}">
                <!-- ステータス条件 -->
                <label for="order_status" class="col-start-6 col-span-1 text-sm text-center py-2 bg-black text-white">ステータス</label>
                <select id="order_status" name="order_status" class="col-span-1 text-sm">
                    <option></option>
                ...
                @foreach($orders as $order)
                    <tr class="text-sm hover:bg-teal-100">
                        ...
                        <td class="p-1 px-2 border text-center">
                            <a href="{{ route('order_detail.index', ['order_id' => $order->order_id]) }}" class="rounded-lg bg-orange-200 text-sm text-center py-1 px-2">詳細</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
